...
import { Search, Filter, SortAsc, Star, ChevronLeft, ChevronRight } from "lucide-react";
...
export function BibliotecaCanciones() {
  const { cargar, cargando, filtros, setFiltros, pagina, setPagina, porPagina, cancionesFiltradas } = useCancionesStore();

  useEffect(() => { cargar(); }, [cargar]);

  const todas = cancionesFiltradas();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <span>{todas.length} canciones encontradas</span>
        {totalPaginas > 1 && (
      ...
      {cargando ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {canciones.map((c) => <CardCancion key={c.id} cancion={c} />)}
        </div>
      )}
